...
const Slider = () => {
    ...
    return (
        <article className="slider_container">
            <img src={ArrowLeft} className="slider_arrow_left" alt="Left arrow icon" onClick={prevSlide} />
            {SliderData.map((slide, index) => {
                return (
                    <div className={index === current ? 'slide active' : 'slide'} key={index}>
                        {index === current && (<img src={slide.image} className="slider_images" alt="Various spa images" />)}
                    </div>
                )
            })}
            <img src={ArrowRight} className="slider_arrow_right" alt="Right arrow icon" onClick={nextSlide} />
        </article>
    )
}
